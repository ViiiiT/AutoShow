...
本项目完全是 Vibe Coding 的产物，即使是README文件也完全由AI生成。代码、交互和渲染流程都在快速迭代中形成，建议在正式使用、公开部署或处理重要素材前自行审查代码、依赖和输出结果。推荐在使用前对代码进行审查，避免发生意外行为。

This project was built entirely through Vibe Coding, even this README file. Please review the code, dependencies, and rendered output before production use or before processing important/private assets. Reviewing codes before use is recommanded to avoid unexpected results.
